...
<body class="flex h-screen bg-slate-950 overflow-hidden">
                ...
                <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    <div class="stat-card">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="badge badge-info">{{ $serverMetrics['cpu_usage'] }}%</span>
                        </div>
                        <div class="w-full bg-slate-700 rounded-full h-2">
                            <div class="bg-gradient-to-r from-primary to-secondary h-2 rounded-full" style="width: {{ $serverMetrics['cpu_usage'] }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="badge badge-warning">{{ $serverMetrics['memory_usage'] }}%</span>
                        </div>
                        <div class="w-full bg-slate-700 rounded-full h-2">
                            <div class="bg-gradient-to-r from-warning to-danger h-2 rounded-full" style="width: {{ $serverMetrics['memory_usage'] }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="badge badge-danger">{{ $serverMetrics['disk_usage'] }}%</span>
                        </div>
                        <div class="w-full bg-slate-700 rounded-full h-2">
                            <div class="bg-gradient-to-r from-danger h-2 rounded-full" style="width: {{ $serverMetrics['disk_usage'] }}%"></div>
                        </div>
                    </div>
                    ...
                    <div class="stat-card">
                        <div class="flex justify-between items-center mb-2">
                            ...
                            <span class="badge badge-success">{{ $serverMetrics['active_connections'] }}</span>
                        </div>
                        <div class="text-2xl font-bold text-primary">{{ $serverMetrics['active_connections'] }}</div>
                    </div>
                </div>
                        ...
                        <table class="w-full">
                            ...
                            <tbody class="divide-y divide-slate-700">
                                ...
                                <tr class="hover:bg-slate-800/50 transition">
                                    <td class="px-6 py-3 text-sm">{{ $log->user?->name ?? 'Système' }}</td>
                                    <td class="px-6 py-3 text-sm"><span class="badge badge-info">{{ $log->action }}</span></td>
                                    <td class="px-6 py-3 text-sm">{{ $log->description }}</td>
                                    <td class="px-6 py-3 text-sm text-slate-400">{{ $log->created_at->diffForHumans() }}</td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
    ...
    <script>
        ...
        new Chart(ctx1, {
            type: 'line',
            data: {
                labels: @json($requestsByDay->pluck('date')),
                datasets: [{
                    label: 'Requêtes',
                    data: @json($requestsByDay->pluck('count')),
                    borderColor: '#f97316',
        ...
        new Chart(ctx2, {
            ...
            data: {
                ...
                datasets: [{
                    data: @json($topModels->pluck('count')),
